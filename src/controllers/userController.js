...
export const becomeCreator = async (req, res) => {
  try {
    ...
    if (req.files) {
      if (req.files.profileImage) profilePath = `/uploads/listings/${req.files.profileImage[0].filename}`;
      if (req.files.coverImage) coverPath = `/uploads/listings/${req.files.coverImage[0].filename}`;
    }
    ...
    res.status(200).json({ message: 'Creator request submitted', user });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
export const updateCreatorProfile = async (req, res) => {
  try {
    if (req.user.role !== 'creator') {
      return res.status(403).json({ message: "Access denied. Only creators can update these fields." });
    }
    ...
    if (req.files) {
      if (req.files.profileImage) updateFields['profile.profileImage'] = `/uploads/listings/${req.files.profileImage[0].filename}`;
      if (req.files.coverImage) updateFields['profile.coverImage'] = `/uploads/listings/${req.files.coverImage[0].filename}`;
    }
